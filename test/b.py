...
class BSpider():
    ...
    def __init__(self):
        ...
        self.browser = webdriver.Chrome(executable_path='E:\test\chromedriver.exe')
    ...
    def get_page(self, upuser_name):
        ...
        for i in range(len(detial_url_list)):
			...
			#将数据写到csv文件中
            with open('bilibiliup.csv', 'a', encoding='utf-8', newline='') as csvfile:
                fieldnames = ['name', 'author', 'date', 'url', 'wachted','bullet_comments','liked','coin','collected','shared','now_date']
                writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
                writer.writerow({'name':namecsv,'author':authorcsv,'date':datecsv,'url':urlcsv,'wachted':wachtedcsv,
