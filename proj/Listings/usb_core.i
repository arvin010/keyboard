...
uint32_t	gnDevState = 0;	 

void *gpCurCfg = 0;						 

PSTD_IF_DSCR	gpCurIf[7];			 
...
void USB_Init(void)
{
  ...
  RCC_USBCLKConfig(((uint32_t)0x00000000));
	...
	USB_ITConfig(((uint32_t) 0x00000402),ENABLE);
	USB_ITConfig(((uint32_t) 0x00000401),ENABLE);
...
void USB_IRQHandler(void)
{
	...
	if(usb_intrusb & ((uint32_t) 0x00000408))						
	{
		((USB_TypeDef *) (((uint32_t)0x40000000) + 0x00005C00))->INTRUSB &= ~0x08;									
		flag1 . bit4 = 1;
	}
	 
	if((usb_intrusb &((uint32_t) 0x00000404)) != RESET)
	{
		USB_Reset();
	}
...
void USB_Suspend(void)
{
	if(usb_state == 0x01)
	{
		((USB_TypeDef *) (((uint32_t)0x40000000) + 0x00005C00))->POWER |= 0x01;
...
void EndpointBulkIn(M_EPBIN_STATUS pbistate, int nCallState)
{
	...
	else
	{
		...
		if(pbistate.nBytesLeft != -1)
		{
			...
			if(pbistate.nBytesLeft < 64)
			{
				nBytes = pbistate.nBytesLeft;
				pbistate.nBytesLeft = -1;
			}
			else 
			{
				nBytes = 64;
...
int EndpointBulkOut(M_EPBOUT_STATUS pbostate, int nCallState)
{
	...
	if(nCallState == 1)
	{
		...
		*((BYTE *)(0x40005c00+14)) = pbostate . byEP;
		byOutCSR = 0x80 | 0x10;
		 
		*((BYTE *)(0x40005c00+20)) = byOutCSR;
	}
	else
	{
		...
		*((BYTE *)(0x40005c00+14)) = pbostate . byEP;
		...
		while(byOutCSR & 0x01)
		{
			...
			nBytes = (int)*((BYTE *)(0x40005c00+23));
			nBytes <<= 8;
			nBytes |= (int)*((BYTE *)(0x40005c00+22));

			 
			if(pbostate.nBytesRecv + nBytes > pbostate.nBuffLen)
			{
				 
				*((BYTE *)(0x40005c00+20)) = 0;
				return nBytes;
			}
...
uint8_t USB_ReadRegister(uint8_t USB_Refister)
{
    ...
    tmp = (uint32_t)((USB_TypeDef *) (((uint32_t)0x40000000) + 0x00005C00));
    tmp += USB_Refister;

     
    return (*(volatile uint8_t *) tmp);	
}
...
void USB_PDCTRLConfig(uint8_t PDCT)
{
    ...
    temp = ((USB_TypeDef *) (((uint32_t)0x40000000) + 0x00005C00))->PDCTRL;
    temp &= ~((uint8_t)(0x03));
    temp |= PDCT;
    ((USB_TypeDef *) (((uint32_t)0x40000000) + 0x00005C00))->PDCTRL = temp;
...
void USB_ITConfig(uint32_t USB_IT, FunctionalState NewState)
{
    volatile uint8_t *tmp = (volatile uint8_t *)(((uint32_t)0x40000000) + 0x00005C00);
    uint32_t IT_FLAG = 0;
    ...
    {
      tmp = &((USB_TypeDef *) (((uint32_t)0x40000000) + 0x00005C00))->INTRINE;
      IT_FLAG = USB_IT & 0xfffffeff;
    }
    else if(USB_IT & 0x00000200)
    {
      tmp = &((USB_TypeDef *) (((uint32_t)0x40000000) + 0x00005C00))->INTROUTE;
      IT_FLAG = USB_IT & 0xfffffdff;
    }
    else if(USB_IT & 0x00000400)
    {
      tmp = &((USB_TypeDef *) (((uint32_t)0x40000000) + 0x00005C00))->INTRUSBE;
      IT_FLAG = USB_IT & 0xfffffbff;
    }

    if(NewState != DISABLE)
    {
      *((volatile uint8_t *)tmp) |= (uint8_t)IT_FLAG;
    }
    else
    {
      *((volatile uint8_t *)tmp) &= ~((uint8_t)IT_FLAG);
...
ITStatus USB_GetITStatus(uint32_t USB_IT)
{
    ...
    if(USB_IT & 0x00000100)
    {
      tmp = &((USB_TypeDef *) (((uint32_t)0x40000000) + 0x00005C00))->INTRIN;
      IT_FLAG = USB_IT & 0xfffffeff;
    }
    else if(USB_IT & 0x00000200)
    {
      tmp = &((USB_TypeDef *) (((uint32_t)0x40000000) + 0x00005C00))->INTROUT;
      IT_FLAG = USB_IT & 0xfffffdff;
    }
    else if(USB_IT & 0x00000400)
    {
      tmp = &((USB_TypeDef *) (((uint32_t)0x40000000) + 0x00005C00))->INTRUSB;
      IT_FLAG = USB_IT & 0xfffffbff;
    }

    tmp_flag = *(volatile uint8_t *)tmp;
...
void USB_Endpoint0(int nCallState)
{
	...
	*((BYTE *)(0x40005c00+14)) = 0;							
	...
	if(byCSR0 & 0x04)
	{
		...
		*((BYTE *)(0x40005c00+17)) = (byCSR0 & ~0x20);
		ep0state.nState = 0;
	}
	...
	if(byCSR0 & 0x10)
	{
		...
		*((BYTE *)(0x40005c00+17)) = (byCSR0 | 0x80);
		ep0state.nState = 0;
	}

	 
	if(ep0state.nState == 0) 
	{
...
void HID_SetReportEvent(PM_EP0_STATUS pep0state,PCOMMAND pcmd)
{
	...
	{
		switch(pcmd->wValue . w >>8)
		{
			case	0x01:break;
			case	0x02:
				 
				c_hid_data.wHID_Status = 0x09;
...
void HID_SetIdleEvent(PM_EP0_STATUS pep0state,PCOMMAND pcmd)
{
	...
	c_hid_data.wHID_Status = 0x0A;

	pep0state->nState = 0;
	c_hid_data.wHID_IdleTimeIf0ID = (pcmd->wValue . w>>8);
	*((BYTE *)(0x40005c00+17)) = 0x40;		
}
...
void HID_SetProtocolEvent(PM_EP0_STATUS pep0state,PCOMMAND pcmd)
{
	...
	c_hid_data.wHID_Status = 0x0B;

	pep0state->nState = 0;
	c_hid_data.wHID_Protocol = (pcmd->wValue . w & 0x00ff);
	*((BYTE *)(0x40005c00+17)) = 0x40;		
}
...
static void USB_Endpoint0_Command(PM_EP0_STATUS pep0state, PCOMMAND pcmd)
{
	...
	switch(pcmd->bmRequestType & 0x60)	
	{
...
{
	int	bError = 0, bNoData = 0, n;
	BYTE	by;
	BYTE	byConfig;
	PSTD_CFG_DSCR	pcfg;	
	
	uint8_t Related_Endpoint;
	uint8_t wIndex0 = pcmd->wIndex . bw . bb0;
	...
	switch(pcmd->bRequest) 
	{
		...
		case 0x05:
			...
			if (pcmd->bmRequestType != 0x00)
			{
				bError = 1;
			}
			else if (gnDevState <= 1) 
			{
				pep0state->byFAddr = (BYTE)pcmd->wValue . w;
				bNoData = 1;
			}
			else
			{
		...
		case 0x06:
			...
			if(!(pcmd->bmRequestType & 0x80))
			{
				bError = 1;
			}
			else 
			{
				switch(pcmd->wValue . w & 0xFF00) 
				{
					...
					case	(0x22<<8):		
						...
						switch(pcmd->wIndex . w)
						{
							case	0:
								
								pep0state->nBytesLeft = USB_HID_FSReportDescriptor_1Size;		
							...
							case	1:
								...
								pep0state->nBytesLeft = USB_HID_FSReportDescriptor_2Size;		
							...
							case	2:
								...
								pep0state->nBytesLeft = USB_HID_FSReportDescriptor_3Size;		
								...
								pep0state->nBytesLeft = USB_HID_FSReportDescriptor_4Size;		
								...
								pep0state->nBytesLeft = USB_HID_FSReportDescriptor_5Size;		
		...
		case 0x0B:
			if(gnDevState > 1)
			{
				...
				{
					gbyCurIfVal[n] = (BYTE)pcmd->wValue . w;
					if (ConfigureIfs() == 0)
					{
						bError = 1;
					}
		...
		case 0x08:
			if ((gnDevState == 1) && (!pcmd->wValue . w))
			{
				 
				pep0state->nBytesLeft = 1;
				pep0state->pData = (void*)&pcmd->wValue . w;
				pep0state->nState = 2;
			}
			else if(gnDevState > 1)
			{
		...
		case 0x03:
			switch (pcmd->bmRequestType) 
			{
					...
					case 0x02:
						...
						switch (pcmd->wValue . bw . bb0) 
						{						
							case 0x0000:
	
							*((BYTE *)(0x40005c00+14)) = Related_Endpoint;	
							if (pcmd->wIndex . bw . bb0 & 0x80)
							{
								if(Related_Endpoint) 
								{
									srbuff = *((uint8_t*)0x40005c00+17);	
									srbuff = srbuff |(0x1<<4);
									*((BYTE *)(0x40005c00+17)) = srbuff;
									INCSRDATA = *((BYTE *)(0x40005c00+17));	
									__asm("NOP");
								}
								...
								{
									srbuff = *((uint8_t*)0x40005c00+17);	
									*((uint8_t*)0x40005c00+17) = srbuff |(0x1<<5);
									srbuff = *((uint8_t*)0x40005c00+17);	
								}
							...
							else
							{
								...
								if(Related_Endpoint)
								{
									srbuff = *((uint8_t*)0x40005c00+20);	
									*((uint8_t*)0x40005c00+20) = srbuff |(0x1<<5);
									srbuff = *((uint8_t*)0x40005c00+20);								
								}
							...
							*((BYTE *)(0x40005c00+14)) = Related_Endpoint;	
							...
							{ 
								if(Related_Endpoint)
								{
									...
									srbuff = *((uint8_t*)0x40005c00+17);	
									*((uint8_t*)0x40005c00+17) = srbuff & (~(0x01<<6));
									srbuff = *((uint8_t*)0x40005c00+17);											
									

									srbuff = *((uint8_t*)0x40005c00+17);	
									*((uint8_t*)0x40005c00+17) = srbuff & (~(0x01<<4));	
									INCSRDATA = *((uint8_t*)0x40005c00+17);	
									__asm("NOP");
								}
								...
								else
								{
									...
									srbuff = *((uint8_t*)0x40005c00+17);	
									*((uint8_t*)0x40005c00+17) = srbuff & (~(0x01<<5));
									srbuff = *((uint8_t*)0x40005c00+17);										
							...
							else
							{
								...
								if(Related_Endpoint)
								{
									...
									srbuff = *((uint8_t*)0x40005c00+20);	
									*((uint8_t*)0x40005c00+20) = srbuff & (~(0x01<<7));
									srbuff = *((uint8_t*)0x40005c00+20);											
									
									srbuff = *((uint8_t*)0x40005c00+20);	
									*((uint8_t*)0x40005c00+20) = srbuff & (~(0x01<<5));
									srbuff = *((uint8_t*)0x40005c00+20);				
		...
		case 0x00:	
			...
			switch (pcmd->bmRequestType) 
			{
					...
					{	
						if(Related_Endpoint){
							*((BYTE *)(0x40005c00+14)) = Related_Endpoint;
							...
							if(*((uint8_t*)0x40005c00+17) & (0x1<<2))
							txdat[0] = 1;	
							else txdat[0] = 0;								
					}
					...
					{
						if(Related_Endpoint){
							if(*((uint8_t*)0x40005c00+20) & (0x1<<5))
								txdat[0] = 1;	
							else txdat[0] = 0;	
						}
						else
...
int USB_EP_Rx(uint8_t Ep,uint8_t *ptr,uint8_t data_len)
{
	...
	tEp1out.byEP = Ep;
	tEp1out.nBuffLen = data_len; 	
	tEp1out.pData = ptr;
	tEp1out.nBytesRecv = 0;

	return EndpointBulkOut(tEp1out, 0);
