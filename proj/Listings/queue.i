...
static void move_write_point(queue_t* queue)
{
    queue->write_point = (queue->write_point + 1) % queue->item_qty;
}
...
static void move_read_point(queue_t* queue)
{
    queue->read_point = (queue->read_point + 1) % queue->item_qty;
}
...
{
    if(queue->read_point == queue->write_point)
    {
        return 1;
    }
    return 0;
...
{
    if((queue->write_point + 1) % queue->item_qty == queue->read_point)
    {
        return 1;
    }
    return 0;
...
int queue_init(queue_t* queue, uint8_t* mem_pool, size_t item_len, size_t item_qty)
{
    ...
    queue->memory = mem_pool;
    if(queue->memory == 0)
    {
        return -2;
    }
    memset(queue->memory, 0, item_len * item_qty);
...
int queue_push(queue_t* queue, void* data)
{
    ...
    uint8_t(*p)[queue->item_len] = (uint8_t(*)[queue->item_len])queue->memory;
    memcpy(p[queue->write_point], data, queue->item_len);

    
    move_write_point(queue);
...
int queue_pop(queue_t* queue, void* recv, size_t len)
{
    ...
    if(queue == 0 || recv == 0 || len < queue->item_len)
    {
        return -1;
    }
    ...
    uint8_t(*p)[queue->item_len] = (uint8_t(*)[queue->item_len])queue->memory;
    memcpy(recv, p[queue->read_point], queue->item_len);

    
    move_read_point(queue);
