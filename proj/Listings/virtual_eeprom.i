# 1 "..\\user\\virtual_eeprom.c"
# 1 "..\\user\\virtual_eeprom.h"
...
uint8_t eeprom_read_byte(eeprom_t* eeprom, size_t addr);
void eeprom_read(eeprom_t* eeprom, size_t addr, uint8_t* data, size_t len); 
...
{
	return *(uint32_t*)(0x800B000 + (addr * 4));
}
...
static uint8_t read_byte(uint32_t addr)
{
	...
	return ( *(uint32_t*)(0x800B000 + (byte4 * 4 * 4)) & ( 0xFF << (byte * 8) ) ) >> ( byte * 8 );
}




static uint8_t read_bytes(uint32_t addr, uint8_t* recv, uint8_t num)
{
	uint8_t i;
	uint8_t conut = 0;
	
	if(recv == 0 || addr + num > 20480)
	{
		return 0;
	}
...
static FLASH_Status write_word(uint32_t addr, uint32_t data)
{
	FLASH_Status status;
	FLASH_Unlock();
	FLASH_ClearFlag(((uint32_t)0x00000020) | ((uint32_t)0x00000004) | ((uint32_t)0x00000010));
	status = FLASH_ProgramWord(0x800B000+(addr * 4), data);
	FLASH_Lock();
	return status;
}
...
static void eeprom_write_nverify(eeprom_t* eeprom, uint32_t addr, const uint8_t* data, size_t len)
{
	...
	start_page = (eeprom->start - eeprom->flash_start) / eeprom->page_size;
	
	 
	page = start_page + addr / eeprom->page_size;
	
	 
	offset = addr % eeprom->page_size;
	
	 
	page_addr = eeprom->flash_start + (page * eeprom->page_size);
	
	 
	read_bytes(page_addr, eeprom->buffer, eeprom->page_size);
	...
	for(i = 0; i < eeprom->page_size / 4; i += 4)
	{
		tdata = *(uint32_t*)(eeprom->buffer + (i * 4));
		write_word(page_addr + (i * 4), tdata);
	}
}
...
static int verify_data(eeprom_t* eeprom, size_t addr, const uint8_t* data, size_t len, page_data_t* pdata)
{
	...
	page_offset = addr % eeprom->page_size;
	
	if( len > eeprom->page_size - page_offset )
	{
...
uint8_t eeprom_read_byte(eeprom_t* eeprom, size_t addr)
{
	...
	return ( *(uint32_t*)(eeprom->start + (byte4 * 4 * 4)) & ( 0xFF << (byte * 8) ) ) >> ( byte * 8 );
}
...
void eeprom_write(eeprom_t* eeprom, size_t addr, const uint8_t* data, size_t len)
{
	...
	else
	{
		...
		eeprom_write_nverify(eeprom, addr, data, len);
	}
}
