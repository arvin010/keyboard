...
uint8_t host_recv_online(queue_t* queue, uint8_t* cmd, uint8_t* recv, size_t* len)
{
	...
	static uint8_t tcmd = 0;
	static uint32_t now = 0;
	if(queue_length(queue))
	{
		queue_pop(queue, &rpkg, sizeof(pkg_st));
		if(rpkg.sn == 0)
		{
			now = GetTime();
			tcmd = rpkg.cmd;
		}
		memcpy((recv + (rpkg.sn * 60)), rpkg.buffer, rpkg.len);
		tlen += rpkg.len;
		if(rpkg.sn == (rpkg.qty - 1))
		{
			*len = tlen;
			*cmd = tcmd;
			tlen = 0;
	...
	if(tcmd != 0 && GetTime() - now >= 5000)
	{
		uint8_t t[sizeof(pkg_st)];
		while(queue_length(queue))
		{ 
			queue_pop(queue, t, sizeof(t));
		}
		tlen = 0;
		tcmd = 0;	
		*len = tlen;
...
uint8_t host_send_online(uint8_t cmd, const uint8_t* send, size_t len)
{
	...
	while(index <= (qty - 1))
	{
		...
		else
		{
			...
			memcpy(data.buffer, send + (index * 60), data.len);
		}

		
		USB_Transmit_VendorData((uint8_t*)&data, 64);
