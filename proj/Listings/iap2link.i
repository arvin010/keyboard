	...
	, uint16 pSessionMgrLen)
{
	...
	uint16 totalLen = pSessionMgrLen + 9;
	...
	pPkt->m_Sof = (uint16)0xff5a;

	pPkt->m_Len = (uint16)(totalLen);

	(*((uint8*)&(pPkt->m_CtrlByte)))	= (*((uint8*)&(ctrl)));
	pPkt->m_SeqNum 		= seq;
	pPkt->m_AckSeqNum 	= ackSeq;
	pPkt->m_SessionId 	= sessionId;
	pPkt->m_HeaderSum	= Iap2LinkPkt_GetCheckSum(pByte, 9-1);

	memcpy(pPkt->m_Payload, pSessionMgr, pSessionMgrLen);
	 pIap2CtrlSessionMsg = (Iap2CtrlSessionMsg *)pSessionMgr;
	 if(pIap2CtrlSessionMsg->m_Param!=0)
	 	if(pIap2CtrlSessionMsg->m_Param->m_Data!=0)
	 		{
			free(pIap2CtrlSessionMsg->m_Param->m_Data);
			pIap2CtrlSessionMsg->m_Param->m_Data = 0;
	 		}
...
void LinkPayload_Init(LinkPayload* pLinkPayload)
{
	...
	pLinkPayload->m_LinkVer 			= 0X01;
	pLinkPayload->m_MaxOfOutstandingPkt = 0X01;
	
	pLinkPayload->m_MaxPktLenMsb 	= (uint8)(((uint16)(32768+30))>>8);
	pLinkPayload->m_MaxPktLenLsb 	= (uint8)((uint16)(32768+30));
	
	pLinkPayload->m_ReTxTimeOutMsb 	= (uint8)(((uint16)1500)>>8);
	pLinkPayload->m_ReTxTimeOutLsb 	= (uint8)((uint16)1500);
	
	pLinkPayload->m_AckTimeOutMsb 	= (uint8)(((uint16)(0))>>8);
	pLinkPayload->m_AckTimeOutLsb 	= (uint8)((uint16)(0));
	
	pLinkPayload->m_MaxReTxCount 	= (10);
...
void Iap2Link_TimeOut(Iap2Link* pIap2Link, uint8 timeId)
{
	if(1 == timeId)
	{
		if((pIap2Link->m_MaxReTxCount != 0xFF)
...
uint8 Iap2Link_Verify(Iap2Link* pIap2Link, Iap2LinkPkt* pPkt, int len)
{
	...
	if(len < 9 || pktLen != len || len > (uint16)(32768+30))
	{
		return 0;
	}

	if(pPkt->m_Sof != (uint16)0xff5a)
	{
		return 0;
	}

	checkSum = Iap2LinkPkt_GetCheckSum(pPkt, 9 - 1);
...
uint8 Iap2Link_TxData(Iap2Link* pIap2Link, const void* pData, int len, uint32 reSendTimeMs, uint8 maxTxCount)
{
	...
	{
		SwTimer_Start(&pIap2Link->m_ReTxTimer, 1, reSendTimeMs);
	}
...
Iap2Rc Iap2Link_Payload_SyncLink(Iap2Link* pIap2Link, Iap2Event event, const Iap2LinkPkt* pPkt)
{
	...
	if(IAP2_EVENT_TRANSFER_SUCCESS == event)
	{
		...
		{
			Iap2Link_RcvData(pIap2Link, pIap2Link->m_pRspPkt, 23);
...
Iap2Rc Iap2Link_SendSyncPkt(Iap2Link* pIap2Link)
{
	...
	i = Iap2LinkPkt_Init(pPkt
		...
		, 0
		, &linkPayload
		, (10) + sizeof(SessionIden) * pIap2Link->m_SessionCount
		);

	Iap2Link_TxData(pIap2Link, pPkt, i, 1000, (10));
...
void Iap2Link_Start(Iap2Link* pIap2Link)
{
	...
	if(!Iap2Link_TxData(pIap2Link, auth_init_data, sizeof(auth_init_data), 1000, 0xFF))
	{
