...
int queue_init(queue_t* queue, uint8_t* mem_pool, size_t item_len, size_t item_qty);
int queue_push(queue_t* queue, void* data);
int queue_pop(queue_t* queue, void* recv, size_t len);
int queue_length(queue_t* queue);
int queue_peek(queue_t* queue, void* recv, size_t len);
...
typedef enum
{
    em_adjust_backlight = 0x01,     
    em_adjust_speed,                
    em_toggle_effect,               
    em_get_current_seb,             
    em_get_effect_table,            
    em_get_macro_table,             
    em_get_keyremap_table,          
    em_send_macro,                  
    em_send_keyremap,               
    em_reset,                       
    em_answer,                      
...
uint8_t host_recv_online(queue_t* queue, uint8_t* cmd, uint8_t* recv, size_t* len);
uint8_t host_send_online(uint8_t cmd, const uint8_t* send, size_t len);
...
extern uint8_t gCurrentEffect;			 
extern uint8_t gCurrentBacklight;		 
extern uint8_t gCurrentSpeed;			 
...
typedef void (*erase_t)(uint32_t addr);
typedef uint8_t (*write_t)(size_t addr, size_t data);
...
uint8_t read_byte(size_t eeprom_start, size_t addr);

void erase_page(uint32_t addr);
uint8_t write_word(size_t addr, size_t data);


void eeprom_init(eeprom_t* eeprom, uint8_t* buffer, size_t len);
void eeprom_config(eeprom_t* eeprom, size_t start, size_t size, size_t flash_start);
void eeprom_register(eeprom_t* eeprom, erase_t erase, write_t write);
size_t eeprom_read(eeprom_t* eeprom, size_t addr, uint8_t* data, size_t len); 
void eeprom_write(eeprom_t* eeprom, size_t addr, const uint8_t* data, size_t len);
...
void eeprom_config(eeprom_t* eeprom, size_t start, size_t size, size_t flash_start)
{
	if( eeprom != 0 && (eeprom->init == 0xBB || eeprom->init == 0xAA) )
	{
		eeprom->size = size;
		eeprom->start = start;
		eeprom->flash_start = flash_start;
		eeprom->init = 0xBB;
	}
}
...
inline uint8_t read_byte(size_t eeprom_start, size_t addr)
{

	uint8_t byte = addr % 4;
	uint32_t byte4 = addr / 4;
	uint32_t d = 0x00;

	d = *(uint32_t*)(eeprom_start + (byte4 * 4));
	return d >> (byte * 8) & 0xFF;
}
...
size_t eeprom_read(eeprom_t* eeprom, size_t addr, uint8_t* data, size_t len)
{
	...
	if( eeprom != 0 && eeprom->init == 0xCC )
	{	
		if( data == 0 || ( addr + len ) > ( eeprom->start + eeprom->size ) )
		{
			return 0;
		}
		...
		{
			data[conut++] = read_byte( eeprom->start, i );
		}
...
static void eeprom_write_page(eeprom_t* eeprom, uint32_t addr)
{
	size_t i; 
	size_t write_addr = addr - addr % eeprom->page_size;
	uint32_t data = 0x00;

	for(i = 0; i < eeprom->page_size; ++i)
	{	
		if(i % 4 == 0)
		{
			data |= eeprom->buffer[i + 3];
			data <<= 8;
			data |= eeprom->buffer[i + 2];
			data <<= 8;
			data |= eeprom->buffer[i + 1];
			data <<= 8;
			data |= eeprom->buffer[i];
			
			write_word( write_addr + i, data); 
			data = 0;
		}
	}	
}
...
static void eeprom_write_nverify(eeprom_t* eeprom, uint32_t addr, const uint8_t* data, size_t len)
{
	int i;
	uint32_t tdata;
	size_t offset, page_addr;
	
	
	 
	page_addr = eeprom->start + ( ( (addr - eeprom->start) / eeprom->page_size ) * eeprom->page_size );	
	for(int i = 0; i < eeprom->page_size; ++i)
	{
		eeprom->buffer[i] = read_byte(page_addr, i);
	}
	
	 
	offset = addr % eeprom->page_size;
	memcpy( eeprom->buffer + offset, data, len );
	
	
	 
	eeprom->erase( addr );
	eeprom_write_page( eeprom, addr);
	
	
	memset(eeprom->buffer, 0xFF, eeprom->page_size);
...
static int verify_data(eeprom_t* eeprom, size_t addr, const uint8_t* data, size_t len, page_data_t* pdata)
{
	...
	size_t current_addr = addr;
	size_t offset = ( addr / eeprom->page_size ) * eeprom->page_size;

	 
	while(1)
	{
		
		current_addr = offset + ( eeprom->start + (page_count * eeprom->page_size) ) + ( current_addr % eeprom->page_size );	
		pdata[page_count].addr = current_addr;
		
		
		pdata[page_count].data = current_data;	
		
		
		current_len = eeprom->page_size - current_addr % eeprom->page_size;	
		write_len += current_len;
		if(write_len >= len)
		{
			if(write_len > len)
			{
				current_len -= write_len - len;
			}
			exit = 1;
		}
		pdata[page_count].len = current_len;
		page_count++;
...
{
	int i, page_num = 0;
	page_data_t pdata[64];
	
	if(eeprom != 0 && eeprom->init == 0xCC)
	{	
		page_num = verify_data(eeprom, addr, data, len, pdata);
		if(page_num > 1)
		{
			 
			for(i = 0; i < page_num; ++i)
			{
				eeprom_write_nverify(eeprom, pdata[i].addr, pdata[i].data, pdata[i].len);
			}
		}
		else
		{
			 
			eeprom_write_nverify(eeprom, pdata[0].addr, pdata[0].data, pdata[0].len);
		}
	}
}
...
void eeprom_free(eeprom_t* eeprom)
{
	if(eeprom != 0 && eeprom->init == 0xCC)
	{
		memset(eeprom, 0x00, sizeof(eeprom_t));
	}
}
