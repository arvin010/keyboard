...
void NVIC_Init(NVIC_InitTypeDef* NVIC_InitStruct)
{
  ...
  if (NVIC_InitStruct->NVIC_IRQChannelCmd != DISABLE)
  {
    ...
    tmppriority = ((NVIC_Type *) ((0xE000E000UL) + 0x0100UL) )->IP[NVIC_InitStruct->NVIC_IRQChannel >> 0x02];
    tmppriority &= (uint32_t)(~(((uint32_t)0xFF) << ((NVIC_InitStruct->NVIC_IRQChannel & 0x03) * 8)));
    tmppriority |= (uint32_t)((((uint32_t)NVIC_InitStruct->NVIC_IRQChannelPriority << 6) & 0xFF) << ((NVIC_InitStruct->NVIC_IRQChannel & 0x03) * 8));    
    
    ((NVIC_Type *) ((0xE000E000UL) + 0x0100UL) )->IP[NVIC_InitStruct->NVIC_IRQChannel >> 0x02] = tmppriority;
    
     
    ((NVIC_Type *) ((0xE000E000UL) + 0x0100UL) )->ISER[0] = (uint32_t)0x01 << (NVIC_InitStruct->NVIC_IRQChannel & (uint8_t)0x1F);
  }
  else
  {
     
    ((NVIC_Type *) ((0xE000E000UL) + 0x0100UL) )->ICER[0] = (uint32_t)0x01 << (NVIC_InitStruct->NVIC_IRQChannel & (uint8_t)0x1F);
  }
}
