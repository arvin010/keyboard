...
# 5 "..\\iap2\\iAp2Session.c"
# 6 "..\\iap2\\iAp2Session.c"
# 7 "..\\iap2\\iAp2Session.c"
# 8 "..\\iap2\\iAp2Session.c"
extern Iap2Link* g_pIap2Link;

uint8 SessionMgr_IsIdle(SessionMgr* pSessionMgr)
{
...
int SessionMgr_FsmTxReq(SessionMgr* pSessionMgr, Iap2Event nEventId, const Iap2LinkPkt* pPkt)
{
	...
	SEGGER_RTT_printf(0,0,"### function=%s line=%d\n",__FUNCTION__,50);
	if(nEventId == IAP2_EVENT_TRANSFER_SUCCESS)
	{
		if(Iap2Link_TxData(g_pIap2Link, pSessionMgr, 1, 0, 0))
		{
...
int SessionMgr_FsmRxReq(SessionMgr* pSessionMgr, Iap2Event nEventId, const Iap2LinkPkt* pPkt)
{
	...
	{
	SEGGER_RTT_printf(0,0,"Receive data is invalid.\n");
		Iap2Link_RcvData(g_pIap2Link, g_pIap2Link->m_pReqPkt, sizeof(Iap2LinkReqPkt));
		return IAP2RC_SUCCESS;
	}
...
int SessionMgr_FsmRxRsp(SessionMgr* pSessionMgr, Iap2Event nEventId, const Iap2LinkPkt* pPkt)
{
	...
	{
	SEGGER_RTT_printf(0,"AckSeqNum[0x%x][0x%x] is not match.\n", pPkt->m_AckSeqNum, g_pIap2Link->m_pReqPkt->m_SeqNum);
		return IAP2RC_FAILED;
	}
...
Iap2Rc SessionMgr_FsmRxAck(SessionMgr* pSessionMgr, Iap2Event nEventId, const Iap2LinkPkt* pPkt)
{
	...
	{
	SEGGER_RTT_printf(0,0,"AckSeqNum[0x%x][0x%x] is not match.\n", pPkt->m_AckSeqNum, g_pIap2Link->m_pReqPkt->m_SeqNum);
		return IAP2RC_FAILED;
	}
	SessionMgr_FsmReset(pSessionMgr);
	pSessionMgr->EventProc(pSessionMgr, IAP2_EVENT_SESSION_DONE);
...
Iap2Rc SessionMgr_Fsm(SessionMgr* pSessionMgr, Iap2Event nEventId, const Iap2LinkPkt* pPkt)
{
	...
	if(IAP2_STATE_TX_REQ == pSessionMgr->m_State) { return SessionMgr_FsmTxReq(pSessionMgr, nEventId, pPkt); };
	if(IAP2_STATE_RX_REQ == pSessionMgr->m_State) { return SessionMgr_FsmRxReq(pSessionMgr, nEventId, pPkt); };
	if(IAP2_STATE_TX_RSP == pSessionMgr->m_State) { return SessionMgr_FsmTxRsp(pSessionMgr, nEventId, pPkt); };
	if(IAP2_STATE_RX_RSP == pSessionMgr->m_State) { return SessionMgr_FsmRxRsp(pSessionMgr, nEventId, pPkt); };
	if(IAP2_STATE_TX_ACK == pSessionMgr->m_State) { return SessionMgr_FsmTxAck(pSessionMgr, nEventId, pPkt); };
	if(IAP2_STATE_RX_ACK == pSessionMgr->m_State) { return SessionMgr_FsmRxAck(pSessionMgr, nEventId, pPkt); };
