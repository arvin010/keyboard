...
uint8 SwTimer_isTimerOutEx(SwTimer* pTimer)
{
	return SwTimer_isTimerOut(pTimer->m_dwInitTicks, GetTime(), pTimer->m_dwTimeoutTicks);
}
...
uint8 SwTimer_isTimerOut(uint32 initTicks, uint32 newTicks, uint32 timeOutTicks)
{
	...
	SEGGER_RTT_printf(0,"### initTicks=%d newTicks=%d timeOutTicks=%d\n",initTicks,newTicks,timeOutTicks);
...
void TimerManager_ResetTimer(TimerManager* pTm, uint32 ticks)
{
	...
	pTm->m_ticks = ticks;
	for(;pTimer != 0; pTimer = (SwTimer*)(((List*)pTimer)->m_pNext))
	{
