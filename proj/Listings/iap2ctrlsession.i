...
# 4 "..\\iap2\\Iap2CtrlSession.c"
# 5 "..\\iap2\\Iap2CtrlSession.c"
# 1 "..\\iap2\\iap2.h"
...
# 6 "..\\iap2\\Iap2CtrlSession.c"
# 7 "..\\iap2\\Iap2CtrlSession.c"
# 8 "..\\iap2\\Iap2CtrlSession.c"
# 9 "..\\iap2\\Iap2CtrlSession.c"
# 10 "..\\iap2\\Iap2CtrlSession.c"

# 12 "..\\iap2\\Iap2CtrlSession.c"
extern Iap2Link* g_pIap2Link;
extern uint8 g_start_key ;
...
void Iap2CtrlSessionMsg_swap(Iap2CtrlSessionMsg* pMsg)
{
	...
	Iap2CtrlSessionMsgParam* pParam = 0;
	
	pMsg->m_Len   = (uint16)(pMsg->m_Len);
	pMsg->m_MsgId = (uint16)(pMsg->m_MsgId);

	while(i < pMsg->m_Len)
	{
		pParam = (Iap2CtrlSessionMsgParam*)&pByte[i];

		pParam->m_Len 	= (uint16)(pParam->m_Len);
		pParam->m_MsgId = (uint16)(pParam->m_MsgId);
	...
	, uint16 len)
{
	...
	pByte[i++] = (uint8)((paramId)>>8);	
	pByte[i++] = (uint8)(paramId);	
	
	memcpy(&pByte[i], pData, len);	
	
	
	
	
	pMsg->m_Len = (uint16)(len);
...
static int Iap2CtrlSession_AuthenticationCerticate(const Iap2CtrlSessionMsg* pReq, uint8* pData, uint16* pLen)
{
	 ...
	 err = i2c_readdata(((I2C_TypeDef *) (((uint32_t)0x40000000) + 0x00005400)),0x20,0x30,buf,2);
	 if(err < 0)
	 {
		 return err;
	 }
	...
	err = i2c_readdata(((I2C_TypeDef *) (((uint32_t)0x40000000) + 0x00005400)),0x20,0x31,certificateBuf,certificateLen);
	 if(err < 0)
	 	{
	   ...
	   unsigned short parameterLen = ( unsigned short)certificateLen+4;
		...
		payload[0]= 0x40;
		 payload[1]= 0x40;
		 payload[2]= (uint8_t)(((messageLen & 0xff00)>> 8)& 0x00ff);
		 payload[3]= (uint8_t)(messageLen & 0x00ff);
		  
		 payload[4]= 0xAA;
		 payload[5]= 0x01;
		 payload[6]= (uint8_t)(((parameterLen & 0xff00) >> 8)& 0xff);
		 payload[7]= (uint8_t)(parameterLen & 0x00ff);
		 payload[8]= 0x0;
		 payload[9]= 0x0;
...
static int Iap2CtrlSession_ChallengeRsp(const Iap2CtrlSessionMsg* pReq, uint8* pData, uint16* pLen)
{
 ...
 err =    i2c_senddata(((I2C_TypeDef *) (((uint32_t)0x40000000) + 0x00005400)),0x20,0x20,buf,2);
	 if(err < 0)
		{
	  ...
	  err = i2c_senddata(((I2C_TypeDef *) (((uint32_t)0x40000000) + 0x00005400)),0x20,0x21,challengReciveBuf,challengLen-6);
	 if(err < 0)
		{
 ...
 err = i2c_senddata(((I2C_TypeDef *) (((uint32_t)0x40000000) + 0x00005400)),0x20,0x10,&buf[0],1);
  if(err < 0)
	{
 ...
 err =i2c_readdata(((I2C_TypeDef *) (((uint32_t)0x40000000) + 0x00005400)),0x20,0x10,&buf[0],1);
  if(err < 0)
	{
	...
	free(challengReciveBuf);
	 ...
	 if(1!=staus)
		...
		return -1;
				...
				buf[ 0 ] = 0;
				buf[ 1 ] = 0;
				  err = i2c_readdata(((I2C_TypeDef *) (((uint32_t)0x40000000) + 0x00005400)),0x20,0x11,buf,2);
					  if(err < 0)
						{
					...
					buf[ 0 ] = 0;
					buf[ 1 ] = 0;
	  err = i2c_readdata(((I2C_TypeDef *) (((uint32_t)0x40000000) + 0x00005400)),0x20,0x20,buf,2);
		  if(err < 0)
			{
		  ...
		  challengReciveBuf = (uint8_t *)malloc(signatureLen);
			err = i2c_readdata(((I2C_TypeDef *) (((uint32_t)0x40000000) + 0x00005400)),0x20,0x12,challengReciveBuf,signatureLen);
							if(err < 0)
								{
...
{
	* pLen = sizeof(iAP2_identificationInformation);  
...
int Iap2CtrlSession_EventProc(Iap2CtrlSession* pSession, Iap2Event eventId)
{
	...
	{
		if(0xAA00 == pSession->m_MsgId
			|| 0xAA02 == pSession->m_MsgId
			|| 0xAA05 == pSession->m_MsgId)
		{
			SessionMgr_RxReq((SessionMgr*)pSession);
		}
		else if(0x1D02 == pSession->m_MsgId)
		{
			SessionMgr_TxReq((SessionMgr*)pSession, 0, 0);
		}
	}
...

{
   ...
   len = 6;
   payload[messageLen]= (uint8_t)(((len & 0xff00) >> 8)& 0xff);
   payload[messageLen+1]= (uint8_t)(len & 0x00ff);
   payload[messageLen+2]= 0x0;
   ...
   payload[messageLen+4]= 0x12;
   payload[messageLen+5]= 0xa8;
...
{	
	...
	uint16 msgId = (uint16)(pReq->m_MsgId);
...
uint8 Iap2CtrlSession_Init(Iap2CtrlSession* pIap2CtrlSession)
{
	...
	pSessionIden->m_ID   = 0x0a;
	pSessionIden->m_Type = 0x00;
	pSessionIden->m_Ver  = 0x01;
